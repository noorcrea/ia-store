...
var NAV_STYLE = 'style="background:#5c6ac4;padding:12px 20px;display:flex;gap:16px;align-items:center;"';
var LINK_STYLE = 'style="color:white;text-decoration:none;font-weight:bold;font-size:14px;padding:6px 14px;border-radius:4px;background:rgba(255,255,255,0.15);"';
...
function mkbtn(id, label, promptText) {
  var esc = promptText.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
...
'  if (!btn || !res) { alert("Element not found: " + btnId); return; }' +
'  btn.disabled = true;' +
'  btn.textContent = "Generation en cours...";' +
'  res.textContent = "";' +
'  setTimeout(function() {' +
'    res.textContent = "Resultat genere pour: " + prompt.substring(0, 60) + "...\n\nLorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.";' +
'    btn.disabled = false;' +
...
app.get('/page-builder', function(req, res) {
  var body = '<label ' + LABEL_STYLE + '>Nom du produit</label>' +
    ...
    mkbtn('pbbtn', 'Generer la page', 'Generer une page produit pour: ') +
...
app.get('/descriptions', function(req, res) {
  var body = '<label ' + LABEL_STYLE + '>Produit a decrire</label>' +
    ...
    mkbtn('dbbtn', 'Generer la description', 'Generer une description pour: ') +
...
app.get('/emails', function(req, res) {
  var body = '<label ' + LABEL_STYLE + '>Sujet de l email</label>' +
    ...
    mkbtn('ebbtn', 'Generer l email', 'Generer un email marketing pour: ') +
...
app.get('/seo', function(req, res) {
  var body = '<label ' + LABEL_STYLE + '>Page ou produit a optimiser</label>' +
    ...
    mkbtn('sbbtn', 'Optimiser le SEO', 'Optimiser le SEO pour: ') +
